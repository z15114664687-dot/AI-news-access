...
const topicRules: Array<{ topic: string; keywords: string[] }> = [
  ...
  { topic: "工具", keywords: ["tool", "tools", "workspace", "search", "shopping", "office", "copilot", "editor", "integration", "workflow", "connector", "connectors", "security", "vulnerability", "intune", "microsoft 365", "plugin", "plugins", "template", "templates", "工具", "应用", "工作区", "搜索", "购物", "集成", "连接器", "插件", "模板", "安全工具", "网络安全", "漏洞", "扫描", "补丁", "工作流"] },
...
export function inferPrimaryTopic(title: string, summary = "", fallback = "工具") {
  ...
  if (isWorkflowToolOrIntegration(title, summary)) return "工具";
...
function isWorkflowToolOrIntegration(title: string, summary: string) {
  const text = `${title} ${summary}`.toLowerCase();
  return /intune|microsoft 365|excel|powerpoint|word|outlook|插件|模板|应用|工作流|workflow|integration|connector|plugin|template/.test(text);
}
